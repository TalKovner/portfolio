...
function toggleSkills(e) {
  ...
  Array.from(document.getElementsByClassName("skillFlex")).forEach(
    (element) => {
  ...
  Array.from(document.getElementsByClassName("skillsHead")).forEach((element) =>
...
function Skills() {
  ...
  const techSkillsData = skills.techSkills.map((skill, i) => (
    <Ball key={i} animationName={`child${i}`}>
      <p>{skill}</p>
    </Ball>
  ));
  const persSkillData = skills.personalSkills.map((skill, i) => (
    <Ball key={i} animationName={`child${i}`}>
      <p>{skill}</p>
    </Ball>
  ));
  return (
    <section className="skills" id="skills">
      <div className="skillMenu" onClick={toggleSkills}>
        <span id="arrow-down" className="arrow-down">
          <i className="fas fa-caret-right"></i>
        </span>
        <div className="heading skills-heading">
